...
//                        let mime_type = self.mimeTypes[file_url.pathExtension.lowercased()]!
    ...
    func getThumbnail(_ photoId: String, thumbnailWidth: Int, thumbnailHeight: Int, quality: Float, type: String, completion: @escaping (_ result: PictureData?) -> Void) {
        self.fetchOptions.predicate = NSPredicate(format: "mediaType == %d",
                                                  type != "video" ? PHAssetMediaType.image.rawValue : PHAssetMediaType.video.rawValue)
        ...
        fetchResult.enumerateObjects({
            ...
            if(type != "video") {
                ...
                PHImageManager.default().requestAVAsset(forVideo: asset, options: nil, resultHandler: { (avAsset: AVAsset?, avAudioMix: AVAudioMix?, info: [AnyHashable : Any]?) in
                    let video_asset = avAsset as! AVURLAsset
                    do {
                        let imgGenerator = AVAssetImageGenerator(asset: video_asset)
                        imgGenerator.appliesPreferredTrackTransform = true
                        let cgImage = try imgGenerator.copyCGImage(at: CMTimeMake(0, 1), actualTime: nil)
                        let thumbnail = UIImage(cgImage: cgImage)
                        let imageData = PhotoLibraryService.image2PictureData(thumbnail, quality: quality)
                        completion(imageData)
                    } catch let error {
