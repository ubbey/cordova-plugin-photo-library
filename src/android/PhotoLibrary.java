...
public class PhotoLibrary extends CordovaPlugin {
				...
				cordova.getThreadPool().execute(new Runnable() {
					public void run() {
						try {
							...
							PhotoLibraryGetLibraryOptions getLibraryOptions = new PhotoLibraryGetLibraryOptions(
									itemsInChunk, chunkTimeSec, includeAlbumData, mediaType, whereClause);

							service.getLibrary(getContext(), getLibraryOptions,
									new PhotoLibraryService.ChunkResultRunnable() {
										@Override
										public void run(ArrayList<JSONObject> library, int chunkNum,
												boolean isLastChunk) {
											try {

												JSONObject result = createGetLibraryResult(library, chunkNum,
														isLastChunk);
				...
				cordova.getThreadPool().execute(new Runnable() {
					public void run() {
						try {
							...
							PhotoLibraryService.PictureData thumbnail = service.getThumbnail(getContext(), photoId,
									thumbnailWidth, thumbnailHeight, quality, "image");
							callbackContext
									.sendPluginResult(createMultipartPluginResult(PluginResult.Status.OK, thumbnail));
				...
				cordova.getThreadPool().execute(new Runnable() {
					public void run() {
						try {
							...
							callbackContext
									.sendPluginResult(createMultipartPluginResult(PluginResult.Status.OK, photo));
				...
				cordova.getThreadPool().execute(new Runnable() {
					public void run() {
						try {
							...
							service.saveImage(getContext(), cordova, url, album,
									new PhotoLibraryService.JSONObjectRunnable() {
										@Override
	...
	public CordovaResourceApi.OpenForReadResult handleOpenForRead(Uri uri) throws IOException {
		...
		if (isThumbnail) {
			...
			String typeStr = origUri.getQueryParameter("type");
			if (typeStr == null) {
				typeStr = "image";
			}
			...
			return new CordovaResourceApi.OpenForReadResult(uri, is, pictureAsStream.getMimeType(), is.available(),
					null);
